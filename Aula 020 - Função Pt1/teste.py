...
def sorteando():
    ...
    while not resultados.isnumeric():
        resultados = str(
    ...
    for sorteando in range(0, resultados):
        ...
        for x in range(0, randint(1, 6)):
            ...
            if var not in temp:
                temp.append(var)
        print(f' "Sorteando" {extenso[len(temp)]}', ('valores da' if len(temp) > 1 else "valor da"),
              f'{sorteando + 1}ªlista: ', end='')
